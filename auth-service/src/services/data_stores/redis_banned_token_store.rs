...
impl BannedTokenStore for RedisBannedTokenStore {
    async fn add_token(&mut self, token: String) -> Result<(), BannedTokenStoreError> {
        ...
        let _:() = self.conn.write().await.set_ex(token_key, value, ttl).map_err(|_| BannedTokenStoreError::UnexpectedError)?;
        Ok(())
    }
    ...
    async fn contains_token(&self, token: &str) -> Result<bool, BannedTokenStoreError> {
        ...
        let is_banned = self.conn.write().await.exists(token_key).map_err(|_| BannedTokenStoreError::UnexpectedError)?;
        Ok(is_banned)
    }
}
